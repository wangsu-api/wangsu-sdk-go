...
| Deployinstance | This interface is used to deploy specified instances. Users need to provide the ID and type of the instance to be deployed (currently only WaitingRoomConnector type is supported), and the interface will return the deployment operation ID. | POST | /api/v2/deployments |