...
| Querydynamicstaticrequests | This API is used to query the number of dynamic and static requests under different protocols for a customer. Users can specify parameters such as date range, aggregation granularity, region, and acceleration type for the query. The API will return the request count data aggregated according to the specified dimensions. | POST | /myview/dyHit |