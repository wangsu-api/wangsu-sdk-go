...
| --- | --- | --- | --- |
| Getrequestsbydomain | Query the number of requests and output the request data at five-minute granularity. | POST | /myview/hit |
...
| Reportrequesthitrateispprovinceservice | This interface allows users to query the hit rate of the number of requests for a domain name in different ISPs and provinces, and supports returning Chinese or English data based on the request header Accept-Language. Users need to provide parameters such as time range and domain name to obtain data in different time dimensions. The returned content includes the hit rate of the number of requests for each domain name in different ISPs and provinces. It helps users analyze the distribution of network requests. | POST | /api/report/request/hit-rate/isp-province |
...
| Reportrequesthttphttpsservice | Query the requests of HTTP and HTTPS per minute for multiple domain in a specified time period. The user enters the start and end time and the domain, and the returned data can be grouped or merged by domain. The interface will return the number of HTTP and HTTPS requests per minute for the domain. | POST | /api/report/request/http/https |
...
| Reportminuteuserrequestispprovinceservice | Query the number of requests for multiple domain names in each ISP and each province, including only Chinese user information | POST | /api/minute/report/domain/request/isp-province |
