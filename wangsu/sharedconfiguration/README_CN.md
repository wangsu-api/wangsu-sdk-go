...
| Addsharecustomizebots | 该接口用于新增一个共享自定义Bot。用户需指定Bot的名称、描述、处理动作以及详细的匹配条件。匹配条件包括匹配键、匹配函数和条件值列表。成功创建后，接口将返回Bot的唯一ID。 | POST | /api/v1/share-customize-bots/add |
| Listsharecustomizebots | 查询共享自定义Bot列表。 | POST | /api/v1/share-customize-bots/get-list |
| Updatesharecustomizebots | 更新共享自定义Bots。 | POST | /api/v1/share-customize-bots/update |
...
| Listsharedratelimitingrules | 该接口用于获取共享配置的频率限制规则列表。用户可根据规则名称进行过滤查询。响应结果包含规则ID、规则名称、描述、生效状态、关联域名数、统计粒度、处理动作、触发阈值、统计周期、生效周期等详细信息。 | POST | /api/v2/share-rate-limit/get-rule-list |
| Updatesharedratelimitingrule | 该接口用于修改共享配置的频率限制规则。用户可根据规则ID、统计粒度、处理动作等参数，更新规则名称、描述、生效周期等配置。 | POST | /api/v2/share-rate-limit/update-rule |
| Createsharedratelimitingrule | 该接口用于新增一条共享配置的频率限制规则。用户需提供统计粒度、处理动作、触发阈值、统计周期以及生效状态等参数，并可详细定义匹配条件和生效周期，以实现精确的频率限制控制。 | POST | /api/v2/share-rate-limit/add-rule |