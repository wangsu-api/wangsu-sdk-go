...
| Getratelimitingrulesforthesharedconfigurationassociatedwithdomain | This API is used to query the rate limiting rules of shared configuration associated with a specified domain. Users need to provide a domain parameter, and the API will return detailed information about all rate limiting rules associated with this domain, including rule names, trigger thresholds, statistical granularity, effective period, and more. | POST | /api/v2/rate-limit/get-relation-by-domain |
| Addratelimitingrule | This interface is used to add a new rate limiting rule. Users can configure trigger thresholds and corresponding actions, such as logging, blocking, or IP blocking, based on various statistical granularities like IP, User-Agent, Cookie, URL, or request headers, combined with matching conditions such as request method, URI, or status code. The interface supports setting the rule's effective period and duration. | POST | /api/v2/rate-limit/add-rule |
| Listratelimitingrules | This interface is used to retrieve a list of rate limiting rules. Users can perform fuzzy queries based on domain and rule name to find matching rate limiting rules. The response will return detailed rule information, including statistical granularity, trigger threshold, action, effective period, and matching conditions. | POST | /api/v2/rate-limit/get-rule-list |
| Updateratelimitingrule | Updates the configuration of an existing Rate Limiting rule. This API allows users to modify parameters such as statistical granularity, trigger thresholds, actions, effective periods, and various matching conditions by providing the rule ID. The updated rule will then be applied to limit specific traffic as configured. | POST | /api/v2/rate-limit/update-rule |
| Getthreatintelligencedomainwhitelist | This interface is used to retrieve the threat intelligence whitelist for specified domains. Users need to provide a list of domains as input. The interface will return the query results, including whitelist details. | POST | /api/v1/common/intelligence/query-whitelist-list |