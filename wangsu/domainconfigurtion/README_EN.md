...
| Updatefingerprintantihotlinkingconfig | This interface is used to modify the fingerprint anti-hotlinking configuration for a specified domain by providing JA4 fingerprints and control actions. It allows for quick blocking or allowing of JA4 fingerprints by configuring allowed or prohibited JA4 fingerprints. Upon successful invocation, the interface will return the operation status and related identification information. | PUT | /api/config/visitcontrol/fingerprint/* |
| Queryfingerprintantihotlinkingconfig | This interface is used to query the fingerprint anti-hotlinking configuration for a specified domain or domain ID. Users need to provide the domain or domain ID as a query parameter. The response will return detailed fingerprint anti-hotlinking rules for the domain, including allowed/prohibited JA4 fingerprints, control actions (403 rejection or 302 redirection) and related redirection URLs, matching patterns, and priority information. | GET | /api/config/visitcontrol/fingerprint/* |