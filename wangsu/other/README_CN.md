...
| Existflowchannel | 查询客户某天的有流量的域名 | POST | /myview/Existflowchannel |
...
| Virtualchannel | 查询泛域名对应的虚拟域名列表 | POST | /myview/Virtualchannel |
| Wafhit | 查询5分钟粒度的waf请求数 | POST | /myview/Wafhit |
| BandwidthUploadPeakRanking | 对频道按照上传带宽峰值排行，可按照边缘上行或中间上行进行排行。输出按照峰值排行后的频道、峰值时间、峰值、总流量 | POST | /myview/BandwidthUploadPeakRanking |
| HitDeny | 查询被拦截请求趋势图，可分拆分频道或合并频道，输出分钟粒度的被拦截请求次数。 | POST | /myview/HitDeny |
...
| Flowappaday | 查询每天的appa总流量 | POST | /myview/Flowappaday |
| Flowappachannel | 查询分频道appa流量，输出每个频道查询周期内的边缘上行、边缘下行、边缘上行+边缘下行的总流量和带宽峰值 | POST | /myview/Flowappachannel |
...
| Livebacksourcequality | 提供直播域名或者频道的回源拉流质量接口，接口只支持GET请求方式 | GET | /QOSS/api/backSourceQuality |
| Streambanning | 查询秒级禁播信息 | POST | /api/Streambanning |
