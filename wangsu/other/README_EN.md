...
| Submitxlwinjecttask | This interface is used for NewBlueNet clients to submit customized injection tasks. Users need to provide the injection content, unique video identifier, slicing options, file source URL, publishing path, operation instructions, and CDN vendor-authorized username and password. The interface will return the task processing results, including successful and failed data. | POST | /content/inject |
| Getprotocolbandwidthandrequestbyispprovince | This interface is used to query bandwidth and request count data differentiated by protocol (HTTP/HTTPS) under specified provinces and ISPs. Users can specify data granularity, a list of domain names (up to 50), a time range, and set the timestamp rounding method. The interface will return the bandwidth, request count, traffic, and corresponding timestamp for each domain under different provinces, ISPs, and protocols. | POST | /cdn/request/protocol-bandwidth-request-isp-province |